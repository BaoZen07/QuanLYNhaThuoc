...
public class ThongKePanel extends JPanel {
	...
	private ColoredButton btnHomeThuoc;
	private ColoredButton btnEndThuoc;
	private ColoredButton btnBeforeThuoc;
	private ColoredButton btnNextThuoc;
	...
	public ThongKePanel(MainFrame mainFrame) {
		...
		tabPane.addTab("Thống kê thuốc", pnlTKThuoc);
	...
	private void addCenterTKThuoc(JPanel pnlCenter) {
		...
		btnHomeThuoc = new ColoredButton(new ImageIcon("Images/double_left.png")); btnHomeThuoc.setBackground(UIConstant.LINE_COLOR);
		btnEndThuoc = new ColoredButton(new ImageIcon("Images/double_right.png")); btnEndThuoc.setBackground(UIConstant.LINE_COLOR);
		btnBeforeThuoc = new ColoredButton(new ImageIcon("Images/left.png")); btnBeforeThuoc.setBackground(UIConstant.LINE_COLOR);
		btnNextThuoc = new ColoredButton(new ImageIcon("Images/right.png")); btnNextThuoc.setBackground(UIConstant.LINE_COLOR);

		btnHomeThuoc.setToolTipText("Trang đầu");
		btnEndThuoc.setToolTipText("Trang cuối");
		btnBeforeThuoc.setToolTipText("Trang trước");
		btnNextThuoc.setToolTipText("Trang kế tiếp");

		Box boxPage = Box.createHorizontalBox();
		boxPage.add(Box.createHorizontalStrut(5));
		boxPage.add(btnHomeThuoc); boxPage.add(Box.createHorizontalStrut(5));
		boxPage.add(btnBeforeThuoc); boxPage.add(Box.createHorizontalStrut(5));
		boxPage.add(btnNextThuoc); boxPage.add(Box.createHorizontalStrut(5));
		boxPage.add(btnEndThuoc); boxPage.add(Box.createHorizontalStrut(5));
	...
	private void taiDuLieuLenBangThuoc(List<Thuoc> dsThuoc, int minIndex) {
		if(minIndex >= dsThuoc.size() || minIndex < 0)
			return;

		lbPageThuoc.setText("Trang " + (minIndex / 20 + 1) + " trong " + ((dsThuoc.size() - 1) / 20 + 1) + " trang.");
		...
		for(int i = minIndex; i < minIndex + 20; i++) {
			...
			SwingUtilities.invokeLater(() -> {
				...
				modelThuoc.addRow(new Object[] {ha != "" ? ha : b , thuoc.getMaThuoc(),thuoc.getTenThuoc(), thuoc.getNhaCungCap() != null ? thuoc.getNhaCungCap().getTenNCC() : null, thuoc.getQuyCachDongGoi(),
	...
	private void addEvent() {
		btnHomeThuoc.addActionListener(e -> {
			if(dsHoaDon != null) {
				taiDuLieuLenBangThuoc(dsThuoc, 0);
			}
		});

		btnEndThuoc.addActionListener(e -> {
			if(dsHoaDon != null) {
				taiDuLieuLenBangThuoc(dsThuoc, dsThuoc.size() - dsThuoc.size() % 20);
			}
		});

		btnBeforeThuoc.addActionListener(e -> {
			if(dsHoaDon != null) {
				taiDuLieuLenBangThuoc(dsThuoc, currentThuocIndex  - 20);
			}
		});

		btnNextThuoc.addActionListener(e -> {
			if(dsHoaDon != null) {
				taiDuLieuLenBangThuoc(dsThuoc, currentThuocIndex + 20);
			}
		});
		...
		btnThongKeThuoc.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				@SuppressWarnings("unchecked")
				List<Thuoc> list = (List<Thuoc>) kq[3];
